...
Each call is wrapped in :func:`llm_call_observability.trace_call` so
prompt/completion tokens, latency, and estimated cost are persisted to JSONL.
...
from .llm_call_observability import trace_call
from .retry_and_circuit_breaker import RetryConfig, retry_transient
...
class StructuredLLM:
    ...
    (``synthetic_generation_workflow.py``, ``generate_synthetic_seed_datasets.py``)
    need no changes.
    """
