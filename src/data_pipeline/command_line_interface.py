...
Sub-commands:
    ...
    scrape       Run keyless collectors (Wikipedia, HuggingFace by default).
                 ...
                 IELTS dataset is sourced via manual seed + synthetic data.
    validate     Run schema validation across data/processed/*.
    quality-process
...
log = logging.getLogger("data_pipeline.command_line_interface")
...
def cmd_extract(args: argparse.Namespace) -> int:
    """Run rule-based raw-to-processed extractors without an LLM."""
    from .evaluation_seed_preparation.raw_to_processed_extractors import (
        jfleg_to_errors,
        wiki_to_industry_vocab,
    )
...
def cmd_validate(args: argparse.Namespace) -> int:
    from .evaluation_readiness_checks.validate_processed_dataset_schemas import (
...
def cmd_audit(args: argparse.Namespace) -> int:
    from .evaluation_readiness_checks.audit_prd_eval_corpus_readiness import (
        main as _audit_main,
    )
...
def cmd_info(_args: argparse.Namespace) -> int:
    ...
    print("  phases:")
    print("    - reference_data_collection")
    print("    - evaluation_seed_preparation")
    print("    - evaluation_readiness_checks")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sc.add_argument(
        ...
        help="Subset of collectors. Default = wikipedia + huggingface "
             "(ielts is opt-in; see docs/data layer/scraping/data_planning.md).",
    )
    ...
    gen = sub.add_parser(
        "generate",
        help="Synthetic data generation (Seed -> Expand -> Judge -> Decontaminate, needs an LLM API key)",
    )
    from .evaluation_seed_preparation.generate_synthetic_seed_datasets import (
        add_arguments as _add_generate_args,
    )
    ...
    au = sub.add_parser(
        "audit",
        help="Check local raw/processed eval corpus readiness against PRD and data docs.",
    )
