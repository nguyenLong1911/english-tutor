"""Readiness audit for A20 eval/seed data assets.

The audit is intentionally local-only. It inspects ``data/raw`` and
``data/processed`` against the benchmark targets stated in ``PRD_v2.md`` and
the data planning docs, but it does not scrape, call APIs, or mutate data.
Passing this audit means the static evaluation/seed corpus is usable; it does
not certify that the live tutor runtime depends on these datasets.
"""
...
def audit() -> dict[str, Any]:
    ...
    checks.append(warn(
        "Mem0 benchmark scale",
        "PRD calls for a 1,000-2,000 facts/user benchmark; seed data remains 100 facts/persona.",
    ))
